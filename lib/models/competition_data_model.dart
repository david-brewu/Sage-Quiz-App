...
class CompetitionDataModel{
  ...
  CompetitionDataModel({this.price,this.title, this.start, this.end, this.duration, this.id, this.index, this.documents});
  CompetitionDataModel.fromMap(DocumentSnapshot data,int index){
  ...
  String formatStart(){
    ...
    var startHour = startTime.hour.toString().length == 1 ? "0${startTime.hour}" : startTime.hour;
    var startMin = startTime.minute.toString().length == 1 ? "0${startTime.minute}" : startTime.minute;
    var starts = "Starts: ${monthNumber2Name(startMonth)} $startDay, $startYear | $startHour:$startMin";
    return starts;
  }
  ...
  String formatEnd(){
    ...
    var endHour = endTime.hour.toString().length == 1 ? "0${endTime.hour}":endTime.hour;
    var endMin = endTime.minute.toString().length == 1 ? "0${endTime.minute}" : endTime.minute;
