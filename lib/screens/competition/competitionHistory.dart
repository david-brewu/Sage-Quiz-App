...
class CompetitionHistory extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return networkProvider.connectionStatus ? StreamBuilder(
      ...
      builder: (context, snapshot){
        ...
        if(docs.length == 0)
          return Center(child: EmptyWidget(msg: "You haven't participated in any competitions"));
        return ListView.builder(
          ...
          itemBuilder: (context, index){
            ...
            return HistoryCard(title: title, dateTaken: dateTaken, score: score, total: total, competitionId: competitionId,);
          },
        );
      },
...
class HistoryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap:(){
        Navigator.of(context).push(CupertinoPageRoute(builder: (context) => ScoreBoard(competitionId,)));
      },
      child: Padding(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: APP_BAR_COLOR, borderRadius: BorderRadius.all(Radius.circular(10.0))),
          child: Column(children: [
            Text("$title", style: MEDIUM_WHITE_BUTTON_TEXT_BOLD,),
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 0),
              child: Text("Date: $day-$month-$year | $hour:$min", style: MEDIUM_DISABLED_TEXT),
            ),
            Text("Your score: ${(100*score/total).round()}%", style: MEDIUM_DISABLED_TEXT),
          ],),
        ),
      ),
    );
